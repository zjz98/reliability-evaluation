...
data=df[['_id','sbaseline','sfindperiod','dtimefind','sbriefdesp','code_related']]#基线版本，问题发现阶段，问题发现时间，服务
...
data = data[['sbaseline','sbriefdesp','code_related']]
...
x2 = data['baselinenum']
x2 = np.array(x2)
y2 = np.exp(-(func(x-1+0.1 , a, b) - func(x-1, a, b)))
plot2 = plt.plot(x2,y2,'b',label = 'ployfit values')
plt.xlabel('x')
plt.ylabel('y')
plt.legend(loc=4)
plt.show()